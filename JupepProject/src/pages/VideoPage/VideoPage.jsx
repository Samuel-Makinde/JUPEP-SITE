...
const VideoPage = () => {
  ...
  const authFetch = AxiosInterceptor();
   const getAllSubjectDetails = async () => {
    setLoading(true)
    try {
      const response = await authFetch.get(`https://jupeb-site-backend.onrender.com/api/v1/get-all`)
      setLoading(false)
                        ...
                        {expanded[`${subject.name}-${part.partNumber}`] && (
                          <div className='w-full mb-[50px] sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-4 md:grid-flow-row-dense bg-white '>
                            {part.videos.length === 0 ? (
                              <p className='text-[16px] md:text-[24px]'>No available videos for now, please check back later.</p>
                            )
                            :
                            (
                            part.videos.map((video) => (
                              <div key={video._id} className='mt-[40px] w-full sm:w-[200px] md:w-[330px] lg:w-[300px] xl:w-[350px]'>
                                <div>
                                  <video src={video.link} className='rounded-[10px]' />
                                </div>
                                <div className='w-full flex justify-between  mt-[10px]'>
                                  <p className='text-[16px] font-semibold w-8/12'>{video.title}</p>
                                  <div className='w-[100px] h-[40px] flex justify-center items-center bg-[#E5E5E5]'>
                                    <div className='w-full flex justify-around' onClick={() => toggleLike(video._id)}>
                                      <div className=' w-3/12'>
                                        <p>
                                          {isLiked(video._id) ? <AiOutlineLike size={28} /> : <AiTwotoneLike size={28} />}
                                        </p>
                                      </div>
                                      <p className='border-x-2 rounded-[40px] border-[#4190EA]'></p>
                                      <div>
                                        <p>
                                          {isDisliked(video._id) ? <BiDislike size={28} /> : <BiSolidDislike size={28} />}
                                        </p>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <p className='w-full  mt-[15px] text-[14px]'>{video.description}</p>
                              </div>
                            )))}
                          </div>
                        )}
