...
const ViewSubject = () => {
  ...
  const togglePdf = (pdfUrl) => {
    // Clone the current pdfData state to avoid directly modifying it
    const updatedPdfData = { ...pdfData };

    // Toggle the pdf visibility for the clicked book
    updatedPdfData[pdfUrl] = !updatedPdfData[pdfUrl];
...
{/*             
              {showPdf && pdfData && (
          <div className="relative w-[100%] h-full mt-4  pt-[3rem] pl-[3rem]">
            <Document file={`https://jupeb-site-backend.onrender.com/${pdfData}`} onLoadSuccess={onDocumentLoadSuccess} className="flex -m-6">
